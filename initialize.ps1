...
Write-Verbose "Replacing 'PolymorphicSerializations' with '$PackageName' in all files..."
Write-Verbose "Also replacing 'PolymorphicSerializations' with '$($PackageName.ToLower())' in all files..."
...
function ProcessFile {
    ...
    try {
        ...
        if ($content -match "PolymorphicSerializations") {
            $content = $content -replace "PolymorphicSerializations", $PackageName
            $hasChanges = $true
        }
        
        if ($content -match "PolymorphicSerializations") {
            $content = $content -replace "PolymorphicSerializations", $PackageName.ToLower()
            $hasChanges = $true
        }
...
Rename-ProjectItems -SearchPattern "PolymorphicSerializations" -Replacement $PackageName -ItemType Directory
Rename-ProjectItems -SearchPattern "PolymorphicSerializations" -Replacement $PackageName -ItemType File
Rename-ProjectItems -SearchPattern "PolymorphicSerializations" -Replacement $PackageName.ToLower() -ItemType Directory
Rename-ProjectItems -SearchPattern "PolymorphicSerializations" -Replacement $PackageName.ToLower() -ItemType File
...
Write-Output "Package name has been changed from 'PolymorphicSerializations' to '$PackageName'"
Write-Output "Lowercase 'PolymorphicSerializations' has been changed to '$($PackageName.ToLower())'" 